...
def jitter_animation(initial_grid, exposure_time, jitter):
    ...
    def stacked_image(end_step):
        ...
        for step in range(end_step):
            grid += update(step)
        image = grid.reshape((sub_size, resolution, sub_size, resolution)).sum(axis=(1, 3))
...
def avg_intensity_to_frame(grid, bkg_plus_dc, read_noise):
    ...
    # Add read noise
    read_noise_array = np.random.normal(loc=0, scale=read_noise, size=(sub_size, sub_size))
    frame += np.round(read_noise_array).astype(int)
    # Add background and dark current noise
    bkg_and_dc_array = np.random.poisson(lam=bkg_plus_dc, size=(sub_size, sub_size))
...
def observed_image(observatory, initial_grid, jitter):
    ...
    if jitter_time <= observatory.exposure_time:
        ...
        for i in range(observatory.num_exposures):
            ...
            frame = avg_intensity_to_frame(intensity_grid, bkg_plus_dc, read_noise)
            image += frame
    ...
    # few exposures.
        ...
        displacements = np.random.normal(scale=jitter * resolution, size=num_jitters)
        del_x_list = np.rint(np.cos(angles) * displacements).astype(int)
        del_y_list = np.rint(np.sin(angles) * displacements).astype(int)
...
def signal_list(observatory, obs_duration, jitter, spectrum):
    ...
    for i in range(num_images):
        ...
        if i == 0:
            ...
            aper = optimal_aperture(image, observatory.single_pix_noise())
...
if __name__ == '__main__':
    ...
    t0 = time.time()
    sig_list = signal_list(tess_geo_obs, 100, jitter, mag_sp)
    t1 = time.time()
    phot_prec = np.std(sig_list) / np.mean(sig_list) * 10 ** 6
    print("Photometric Precision for mag", mag, ": ", format(phot_prec, '4.0f'), "ppm")
    ...
    # t2 = time.time()
    # mean_list, std_list = multithread_signal_list(tess_geo_obs, 36000, jitter, sp_list)
    # t3 = time.time()
    # phot_prec_list = (std_list / mean_list) * 10 ** 6
    # for ind, mag in enumerate(mag_list):
    #     ppm = phot_prec_list[ind]
    #     print("Photometric precision for mag", format(mag, '2d'), ": ", format(ppm, '4.0f'), "ppm")
    
    # print()
    # print("Time to do a single precision estimate: ", format(t1 - t0, '4.3f'))
    # print("Time to do one precision estimate, with multithreading: ", format((t3 - t2) / len(mag_list),'4.3f'))

    # Run animation
    base_grid = tess_geo_obs.avg_intensity_grid(mag_sp, pos=[0,0], subarray_size=sub_size, resolution=resolution)
